...
transformFast ::    
  ...
  -> AffineMatrix 
  -> AABB
transformFast !aabb !m = newAABB (center' |-> (-extent'')) (center' |-> extent')
  where
    (v0,v1)       = (aabb^.vMin,aabb^.vMax)
    (v0x,v0y,v0z) = (getX v0,getY v0,getZ v0)
    (v1x,v1y,v1z) = (getX v1,getY v1,getZ v1)
...
class WithAABB a where
  ...
  -- object
  extentOf ::    
       a                  -- ^ The object to compute the extent of
    -> (PointR3, PointR3)
  ...
  --
  centroidOf ::    
       a       -- ^ The object to find the centroid of
    -> PointR3
  centroidOf !obj = point3 cx cy cz
    where
      (p,p')        = extentOf obj
  ...
  findCentroid ::     
       [a]     -- ^ The list of points to find the centroid of
    -> PointR3
  findCentroid !objs = 
    (mconcat $ map centroidOf objs) /. (fromIntegral $ length objs :: Double)


  -- | Computes the axis-aligned bounding box (AABB) for the given object
  boundingBoxOf ::
       a    -- ^ The object to compute the AABB of
    -> AABB
  boundingBoxOf !obj = newAABB (point3 x y z) (point3 x' y' z')
    where
      (minP,maxP) = extentOf obj
  ...
  findAABB ::    
       [a]        -- ^ A list of objects to compute the AABB of
    -> Maybe AABB
  findAABB []    = Nothing
  findAABB !objs = Just $ newAABB finalMin finalMax
    where 
      (finalMin,finalMax) = 
        foldr compareAABB (mempty,mempty) objs
      compareAABB obj (currMin,currMax) = (newMin,newMax)
        where
          aabb'           = boundingBoxOf obj
