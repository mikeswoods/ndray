...
module Raytracer.Surface.Texture.Image (ImageTexture, imagemap)
where
...
import Raytracer.Surface.Texture (Texture(..), WithTexture(..), TextureError(..))
...
instance Eq ImageTexture where
  ...
  (==) (ImageMap path1 _ _) (ImageMap path2 _ _) = path1 == path2
...
imagemap ::     
     FilePath      -- Path to the texture file
  -> IO Texture
imagemap filePath = imagemap' filePath >>= return . Texture

--------------------------------------------------------------------------------

instance WithTexture ImageTexture where

  texel (ImageMap _ (w,h) img) (u,v) = 
    ...
    where
      ...
      withInterpPx = bilerp img (u,v) w h w' h'

--------------------------------------------------------------------------------

-- | Reads the pixel value at the given coordinate
readPixelValue ::    
     DynamicImage -- ^ The image to read from
...
readPixelValue (ImageY16 _) i j   = Left $ printf "(%d,%d) Not supported: ImageY16" i j
readPixelValue (ImageYF _) i j    = Left $ printf "(%d,%d) Not supported: ImageYF" i j
readPixelValue (ImageYA8 _) i j   = Left $ printf "(%d,%d) supported: ImageYA8" i j
readPixelValue (ImageYA16 _) i j  = Left $ printf "(%d,%d) supported: ImageA16" i j
readPixelValue (ImageRGBF _) i j  = Left $ printf "(%d,%d) supported: ImageRGBF" i j
...
readPixelValue (ImageRGB16 _) i j  = Left $ printf "(%d,%d) supported: ImageRGB16" i j
readPixelValue (ImageRGBA16 _) i j = Left $ printf "(%d,%d) supported: ImageRGBA16" i j
readPixelValue (ImageYCbCr8 _) i j = Left $ printf "(%d,%d) supported: ImageYCbCr8" i j
readPixelValue (ImageCMYK8 _) i j  = Left $ printf "(%d,%d) supported: ImageCMYK8" i j
readPixelValue (ImageCMYK16 _) i j = Left $ printf "(%d,%d) supported: ImageCMYK16" i j
