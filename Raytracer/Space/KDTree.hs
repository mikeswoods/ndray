...
module Raytracer.Space.KDTree (KDTree(..)
  ,KPoint(..), KVector, KExtent(..), KHits(..)
  ,leftTree, rightTree, aabb, treeDepth, splitCost, splitAxis, numChildren
  ,kdtree, nodeExtent)
where
...
cheapestSplitAxis !k !center !pointsObjs = 
  minimumBy compareCost $ map perAxis [0 .. (k - 1)]
  where
    ...
    perAxis axis = (totalCost,axis,leftOfAxis,rightOfAxis)
      where 
        ...
        (leftPoints,rightPoints) = (map fst leftOfAxis,map fst rightOfAxis)
...
build !k (!depth,!maxDepth) !maxPerChildrenNode !pointsObjs
   | (null pointsObjs) || 
      (depth >= maxDepth) || 
        (countChildren < maxPerChildrenNode) = Leaf $ map snd pointsObjs
