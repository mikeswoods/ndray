...
import Raytracer.Space.Tensor (vec3, point3, cross, unit, (/=>))
...
sphere ::
  ...
  -> IO (WorldObject Primitive)
sphere seqId radius attrs =
  return $ createObject seqId "Sphere" [(Sphere radius)] attrs


plane ::
     Int        -- ^ Object sequence ID
  -> Coordinate -- ^ The (x,y,z) normal of the plane
  -> Double     -- ^ The distance from the origin of the plane
  -> Attributes -- ^ The attributes of the plane
  -> IO (WorldObject Primitive)
plane seqId (nx,ny,nz) dist attrs = 
  return $ createObject seqId "Plane" [obj] attrs
  where
...
triangle ::
     Int        -- ^ Object sequence ID
  -> Coordinate -- ^ The (x,y,z) coordinate of a vertex of the triangle
  -> Coordinate -- ^ The (x,y,z) coordinate of a vertex of the triangle
  -> Coordinate -- ^ The (x,y,z) coordinate of a vertex of the triangle
  -> Attributes -- ^ The attributes of the box
  -> IO (WorldObject Primitive)
triangle seqId (xv0,yv0,zv0) (xv1,yv1,zv1) (xv2,yv2,zv2) attrs = 
  return $ createObject seqId "Triangle" [obj] attrs
  where
...
box ::
     Int        -- ^ Object sequence ID
  -> Coordinate -- ^ The (x,y,z) coordinate of the box's minimum corner
  -> Coordinate -- ^ The (x,y,z) coordinate of the box's minimum corner
  -> Attributes -- ^ The attributes of the box
  -> IO (WorldObject Primitive)
box seqId (xMin, yMin, zMin) (xMax, yMax, zMax) attrs = 
  return $ createObject seqId "Box" [obj] attrs
  where
...
mesh seqId modelFilePath autoScale attrs = do 
  ...
  where
    ...
    primitives meshData = map mkTriangle $ WF.triangles meshData
